...
const crearTasa = async(req, res) => {
    ...
    try {
        ...
        const tasaActualizado = await TasaEurobcv.findOneAndUpdate(
            { usuario: uid }, 
        ...
        res.json({
            ok: true,
            tasa: tasaDB,
            perfil: 'TasaEurobcv actualizado con la nueva tasa'
        });
...
const borrarTasa = async(req, res) => {
    ...
    try {
        const tasaDB = await TasaEurobcv.findById(id);
        if (!tasaDB) {
        ...
        // Borrar el documento
        await TasaEurobcv.findByIdAndDelete(id);
